...
# device = torch.device("cuda")
# dtype = torch.float32
# gradcam = GradCAM(model=model, device=device, dtype=dtype, mode="top-3")

# # 결과를 저장할 디렉토리 경로 설정
# input_dir = "c:\\Users\\AAA\\Deep-Learning-as-Statistical-Perspective\\data\\gradcam_original_test_large"
# results_dir = input_dir + "\\test_or_results"

# # 디렉토리가 존재하지 않으면 생성
# if not os.path.exists(results_dir):
#     os.makedirs(results_dir)

# path1= input_dir + "\\test_or"
...
#     # print(f"The shape of image {filename} is: {img_array.shape}")  # 이미지 shape 출력  
...
# 결과를 저장할 디렉토리 경로 설정
input_dir = "c:\\Users\\AAA\\Deep-Learning-as-Statistical-Perspective\\data\\gradcam_original_test_large"
results_dir = input_dir + "\\test_tf_results"
...
for i, filename in enumerate(lst2, start=1):
    ...
    img_array = np.array(img)
    print(f"The shape of image {filename} is: {img_array.shape}")  # 이미지 shape 출력
    if img_array.shape[2] == 4:  # 알파 채널이 있는지 확인
        img = img.convert('RGB')  # 알파 채널 제거
        img.save(img_path)  # RGB 포맷으로 저장    
    globals()[f'img_path{i}'] = img_path
